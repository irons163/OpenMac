...
struct AgentTaskExecutorTests {
    ...
    func projectsDirectoryResolverBuildsBoardScopedFolder() {
        ...
        let collapsedSeparators = CodexProjectsDirectorySettings.boardScopedProjectsDirectoryPath(
            baseDirectoryPath: base,
            boardName: "A///B:::C"
        )
        #expect(collapsedSeparators == "/tmp/openmac-projects-root/A-B-C")

        let underscoreDashOnly = CodexProjectsDirectorySettings.boardScopedProjectsDirectoryPath(
            baseDirectoryPath: base,
            boardName: "__--__"
        )
        #expect(underscoreDashOnly == "/tmp/openmac-projects-root/board")
    }
...
struct KanbanPersistenceTests {
    ...
    func runPMAutopilotInBackgroundEndToEnd() {
        ...
        #expect(viewModel.lastBoardMessage?.contains("\(L10n.string("Roadmap")) [\(L10n.string("Epic"))]:") == true)
        #expect(viewModel.lastBoardMessage?.contains("\(L10n.format("Milestone: %@", "M2 MVP Complete")) 1/1") == true)
        #expect(viewModel.lastBoardMessage?.contains("\(L10n.format("Epic: %@", "Core Product")) 1/1") == true)
    }
