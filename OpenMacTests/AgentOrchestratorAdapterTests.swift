...
struct AgentOrchestratorAdapterTests {
    ...
    @Test("a failed shared identity probe is cleared before retry")
    func failedSharedIdentityProbeCanRetry() async throws {
        let serviceUnavailable = Data(
            """
            {
              "error": "daemon restarting"
            }
            """.utf8
        )
        let stubs = try AgentOrchestratorAdapterTestFixture.commonStubs([
        ...
        let requests = await transport.requests()
        #expect(
            requests.filter { $0.path == "/healthz" }.count == 3
        )
        #expect(
            requests.filter { $0.path == "/readyz" }.count == 2
        )
