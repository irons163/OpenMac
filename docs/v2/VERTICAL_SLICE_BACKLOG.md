...
- [ ] VS-08：已保存 reconcile failure 與 stop acknowledgement，Control Center 對可恢復 backend 會在重啟載入時 reconcile，也可手動重試；相同 AO snapshot 不重播 facts，backend down／未知狀態顯示 `Unknown`／`Needs You`，stop receipt 不會假裝 session 已終止。2026-08-02 的 live start／stop smoke，以及 AO daemon PID `8874 → 11085` 重啟後的 compatibility smoke 均通過；尚欠 App restart reconcile 與可設定且驗證過的 AO dashboard deep-link。
...
- [ ] 下一步：取得 backend-confirmed verification workspace identity 並跑 Xcode／PR E2E；再補 AO dashboard deep-link 與 restart reconcile smoke。AO live isolated-session 已完成，乾淨 Mac 與受測者／concierge 驗證已明確延後，不阻塞本輪技術進度。
