...
- [ ] VS-08：已保存 reconcile failure 與 stop acknowledgement，Control Center 對可恢復 backend 會在重啟載入時 reconcile，也可手動重試；scene active 時會再次載入並 reconcile，相同 AO snapshot 不重播 facts，deterministic restart test 已用新的 model instance 驗證，backend down／未知狀態顯示 `Unknown`／`Needs You`，stop receipt 不會假裝 session 已終止。2026-08-02 的 live start／stop smoke、AO daemon PID `8874 → 11085` 重啟後的 compatibility smoke，以及 `tools/test-packaged-app-restart.sh` 的 packaged launch → terminate → relaunch → terminate smoke 均通過；尚欠可設定且驗證過的 AO dashboard deep-link。最新 upstream `main` (`9159a020`) 仍沒有可供外部 app 使用的 dashboard HTTP route；`/preview` 是瀏覽器 preview，不當作 dashboard deep-link。
...
- [ ] 下一步：補 AO dashboard deep-link。AO live isolated-session、backend-confirmed workspace identity、真實 Xcode verifier build、live PR facts、scene-active reconcile、deterministic restart coverage 與 packaged restart smoke 已完成；乾淨 Mac 與受測者／concierge 驗證已明確延後，不阻塞本輪技術進度。
