...
- [ ] 下一步：取得 backend-confirmed verification workspace identity 並跑 Xcode／PR E2E；再補 packaged-App restart smoke 與 AO dashboard deep-link。AO live isolated-session、scene-active reconcile 與 deterministic restart coverage 已完成，乾淨 Mac 與受測者／concierge 驗證已明確延後，不阻塞本輪技術進度。
...
| 7 | VS-08：AO reconcile 與 attention | P0 | 輪詢／訂閱必要 facts，映射 running/blocked/failed/ready；restart reconcile；deep-link 到 AO 處理 terminal/follow-up | VS-05、VS-07 | live start／stop smoke 已通過；尚欠 App restart reconcile 與可設定的 dashboard deep-link；未知 AO state 顯示 Unknown/Needs You，不假裝成功 |
