...
- [ ] 下一步：取得 backend-confirmed verification workspace identity 並跑 Xcode／PR E2E；再補 AO dashboard deep-link。AO live isolated-session、scene-active reconcile、deterministic restart coverage 與 packaged restart smoke 已完成，乾淨 Mac 與受測者／concierge 驗證已明確延後，不阻塞本輪技術進度。

VS-01～VS-11 contract path 目前由 151 個測試覆蓋，其中 Xcode verifier suite 包含一個真實 Swift package `xcodebuild` smoke；AO discovery suite 以 8 個 deterministic tests 覆蓋有效檔案、missing、stale、invalid identity、writable、oversized、malformed timestamp 與 symlink，install-readiness suite 也驗證 discovery PID 會綁定 connection probe。Contract tests 不啟動真實 Codex 或 AO；另有 opt-in live smoke 記錄於 [`AO_LIVE_SMOKE.md`](AO_LIVE_SMOKE.md)。Xcode 26 的 hosted runner 在本機會卡在 worker materialization，這是 runner 層問題，未算作產品測試通過。測試不改動舊 Kanban schema。初始 generation 以 3–5 tasks 作為品質目標；編輯後的產品 plan 允許 3–7 tasks，approval eligibility 一律從目前 typed plan 與未解 generation blockers 重算。
