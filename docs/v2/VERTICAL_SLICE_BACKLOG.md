...
- [x] VS-07：AO reference adapter 已完成 health／readiness／served OpenAPI version 與必要 operation 驗證、project selection、idempotent start、snapshot facts 與 stop 的隔離實作；28 個 captured contract tests 固定於 upstream revision `9caafbee89383c9bf7e904936eb88c48add2fa88`，並覆蓋 snapshot 去重、未知狀態 fail-closed、明確 terminated mapping、尚未 ready 時停止 compatibility probe、缺少必要 API operation 時降級、首次併發操作共用完整 compatibility probe、快取相容性前重新確認 daemon identity、併發操作共用 in-flight identity probe、完整或 identity 共享 probe 失敗後重新執行完整 compatibility check、unsafe permission scope 在任何 AO side effect 前拒絕、existing session receipt identity 驗證、terminal stop 不重送 kill、unsafe project／execution path identifier 在任何 request 前拒絕、手動 URL 替換 PID 後重跑完整 probe、discovered connection 替換 PID 後要求重新探索，以及 health／ready／discovery PID 不一致時 fail closed。另依 upstream `b58bae51bac08c9e48bded4c636e504863a93c21` 的 `~/.ao/running.json` 契約新增安全本機探索：拒絕 symlink、非本人擁有、可被 group／other 寫入、過大、格式錯誤或 stale PID 的檔案，只建立 `127.0.0.1` URL，且不讀取或保存 browser runtime token。2026-08-02 已在 AO desktop daemon 與 `tmux 3.7b` 上通過 health、ready、served OpenAPI、project discovery，以及 disposable project 的 fake isolated session start、facts、stop acknowledgement；request／branch／execution identity 均通過。AO 官方 shell-terminal API 也已用於解析並釋放 backend-confirmed `workingDir`；workspace identity 的 adapter test 與 live smoke 均通過。真實 agent 與 Xcode／PR E2E 仍由 VS-09 覆蓋。
- [x] VS-08：已保存 reconcile failure 與 stop acknowledgement，Control Center 對可恢復 backend 會在重啟載入時 reconcile，也可手動重試；scene active 時會再次載入並 reconcile，相同 AO snapshot 不重播 facts，deterministic restart test 已用新的 model instance 驗證，backend down／未知狀態顯示 `Unknown`／`Needs You`，stop receipt 不會假裝 session 已終止。2026-08-02 的 live start／stop smoke、AO daemon PID `8874 → 11085` 重啟後的 compatibility smoke，以及 `tools/test-packaged-app-restart.sh` 的 packaged launch → terminate → relaunch → terminate smoke 均通過；已補可設定的 dashboard root、project/session route builder 與 HTML verification，Control Center 未通過 verification 不會開啟 route。最新 upstream `main` (`9159a020`) 的官方 web renderer 已在 `127.0.0.1:3000` 提供 hash dashboard route；`tools/test-agent-orchestrator-dashboard.sh` 以 OpenMac 真實 `URLSession` verifier 驗證 `openmac-ao-fixture` project 與 `openmac-ao-fixture-1` session route 都回傳 `200 text/html`。安裝的 AO desktop 本身仍只有 `127.0.0.1:3001` daemon、沒有外部 URL scheme；使用者需另外啟動官方 web renderer，Control Center 才會通過 route probe。
- [x] VS-09：已新增獨立 `XcodeVerifier`、Control Center 驗證動作與 atomic evidence persistence；執行前核對 backend-confirmed workspace、branch、Git common directory 與 container，命令以 argument array 啟動並保存 scheme、command、exit status、bounded summary、時間及 `.xcresult`。真實 Swift package smoke 已產生並 round-trip 保存 build record；失敗 evidence 會立即進 `Needs You` 且不能成為 `Ready to Merge`。AO 的 `SessionView` 仍未暴露 `workspacePath`／`workspaceRepoPath`，但官方 `POST /api/v1/shell-terminals` + `DELETE /api/v1/shell-terminals/{handleId}` 已能提供並釋放 backend-confirmed `workingDir`；adapter 會驗證 project／session identity、safe handle 與 absolute path，失敗則 fail closed。2026-08-02 已用 disposable AO run 讓 `XcodeVerifier` 在 backend-confirmed worktree 內完成真實 Swift package `xcodebuild` build，並透過 AO 官方 PR claim endpoint 綁定公開 GitHub PR `Untrivial-ai/agent-orchestrator#3451`，由同一 execution identity 讀回 URL、open state、passing CI 與 required-review facts。PR creation／push／merge／review mutation 不在本 smoke 範圍。
...
- [x] VS-11：已決定 invited-evaluator license、版本 `0.1.0 (3)` 與最低 macOS 14；Release feature flag 會包含 v2 review、control center 與安全 AO discovery／connection probe。封裝工具會拒絕未 commit 的 source，產出 arm64／x86_64 zip、SHA-256、build metadata，並在封裝前後嚴格驗證 ad-hoc hardened-runtime 簽章。test.3 archive 以 `BUILD-INFO.json` 綁定 exact clean source commit，並以同名 `.sha256` sidecar 作為該次 build 的 authoritative checksum；已通過 checksum、解壓、簽章、launch smoke 與 packaged restart smoke。乾淨測試機 Gatekeeper onboarding 依本輪範圍決策延後，不阻塞此 ticket。
- [x] VS-12：已在 disposable Swift package AO fixture 重跑完整 technical E2E：typed 3-task DAG 先保留兩個平行 root reservations，再建立 dependent join；三個真實 AO isolated sessions 都取得 backend-confirmed workspace identity，三個 workspaces 通過 `XcodeVerifier`，join session 由 AO official claim endpoint 讀回公開 PR 的 URL／open／passing CI／required review facts，並輸出 privacy-filtered funnel JSON（3 tasks、3 sessions、3 verified tasks、5 passed evidence、1 PR）。目前 AO project summary 未暴露 permission scope，production `DeliveryDispatcher` 對 `unknown` 仍 fail closed；live smoke 以已批准 store reservations 直接驗證真實 AO backend，dispatcher preflight 與平行 contract 仍由 deterministic tests 覆蓋。
- [x] 技術 slice 完成：AO live isolated-session、backend-confirmed workspace identity、真實 Xcode verifier build、live PR facts、3-task／2-parallel technical E2E、scene-active reconcile、deterministic restart coverage、packaged restart smoke，以及官方 web renderer dashboard HTML route probe 均已完成；乾淨 Mac 與受測者／concierge 驗證已明確延後，不阻塞本輪技術進度。

VS-01～VS-11 contract path 目前由 154 個測試覆蓋，其中 Xcode verifier suite 包含一個真實 Swift package `xcodebuild` smoke；AO discovery suite 以 8 個 deterministic tests 覆蓋有效檔案、missing、stale、invalid identity、writable、oversized、malformed timestamp 與 symlink，install-readiness suite 也驗證 discovery PID 會綁定 connection probe。Contract tests 不啟動真實 Codex 或 AO；另有 opt-in live smoke 記錄於 [`AO_LIVE_SMOKE.md`](AO_LIVE_SMOKE.md)。Xcode 26 的 hosted runner 在本機會卡在 worker materialization，這是 runner 層問題，未算作產品測試通過。測試不改動舊 Kanban schema。初始 generation 以 3–5 tasks 作為品質目標；編輯後的產品 plan 允許 3–7 tasks，approval eligibility 一律從目前 typed plan 與未解 generation blockers 重算。
...
| 6 | VS-07：AO reference adapter spike | P0 | 僅實作 slice 所需的 health/version、project selection、start session 與 facts；所有 DTO 隔離於 adapter | VS-02、Gate A | captured fixtures 與 live AO daemon contract tests 通過；disposable project 可建立並停止 1 個 fake isolated session；不把 AO 型別洩漏到 domain/UI；不相容時顯示版本與可行動錯誤 |
| 7 | VS-08：AO reconcile 與 attention | P0 | 輪詢／訂閱必要 facts，映射 running/blocked/failed/ready；restart reconcile；deep-link 到 AO 處理 terminal/follow-up | VS-05、VS-07 | live start／stop、scene-active reconcile、deterministic restart 與 packaged restart smoke 已通過；dashboard deep-link 已由官方 web renderer route probe 驗證；未知 AO state 顯示 Unknown/Needs You，不假裝成功 |
...
| 9 | VS-11：可安裝測試 build | P1 | 決定 license、最低 macOS、版本號；產出不需 Xcode 的 zip/dmg，提供最短 onboarding | VS-10 | test.3 archive 通過 checksum、簽章、架構與本機 launch smoke；未有簽章 credentials 時明確標示測試安裝步驟，不宣稱 notarized；乾淨測試機 Gatekeeper 驗證延後 |
