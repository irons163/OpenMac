...
- [ ] VS-11：已決定 invited-evaluator license、版本 `0.1.0 (1)` 與最低 macOS 14；Release feature flag 會包含 v2 review、control center 與 AO connection probe。封裝工具會拒絕未 commit 的 source，產出 arm64／x86_64 zip、SHA-256、build metadata，並在封裝前後嚴格驗證 ad-hoc hardened-runtime 簽章。本機 dirty-source 驗證包已完成解壓與 launch smoke；尚欠從已 commit source 產生正式測試包並在乾淨測試機走過 Gatekeeper onboarding，故不把 ticket 標成完成。
- [ ] 下一步：從已 commit source 產生 VS-11 clean archive 並完成乾淨測試機 smoke；之後在相容 AO daemon 完成 VS-07～VS-09 live smoke、dashboard deep-link、permission／verification workspace identity 與 VS-12 Gate B。
